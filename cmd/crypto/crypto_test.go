...
import (
	...
	"github.com/bearguy/saito-go/cmd/crypto"
	"github.com/btcsuite/btcd/btcec"
)

func TestToBase58(t *testing.T) {
	hexString := "22a47fa09a223f2aa079edf85a7c2d4f87"
	encodeToBase58String := crypto.ToBase58(hexString)
...
func TestFromBase58(t *testing.T) {
	...
	decodeFromBase58String := crypto.FromBase58(base58String)
...
func TestDoubleHashB(t *testing.T) {
	...
	messageHash := crypto.DoubleHashB([]byte(message))
...
func TestCompressPublicKey(t *testing.T) {
	...
	compressedPubKey, err := crypto.CompressPubKey(hexString)
	if err != nil {
		fmt.Println(err)
	}
...
func TestUncompressPublicKey(t *testing.T) {
	...
	decodeFromBase58String, err := crypto.UncompressPubKey(base58String)
	if err != nil {
		fmt.Println(err)
	}
...
func TestGenerateKeys(t *testing.T) {
	privateKey, _ := crypto.GenerateKeys()
	privateKeyString := crypto.ConvertPrivKeyToString(privateKey)
...
func TestReturnPublicKey(t *testing.T) {
	...
	pubKey := crypto.ReturnPublicKey(privKey)
	publicKeyString := crypto.ConvertPubKeyToString(pubKey)
...
func TestSignMessage(t *testing.T) {
	privKey, _ := crypto.GenerateKeys()
	pubKey := crypto.ReturnPublicKey(privKey)
	msg := "this is it"
	signature, messageHash := crypto.SignMessage(msg, privKey)

	//verified := signature.Verify(messageHash, pubKey)
	verified := crypto.VerifyMessage(messageHash, pubKey, signature)
